...
R5 adds a provider-neutral memory-wall runtime that executes deterministic workloads through Dense and PrimeStabilizer, records logical representation bytes separately from process RSS, and emits machine-readable receipts suitable for repeated local/cloud experiments. Every benchmark receipt is built only from a clean Git checkout and is bound at build time to the exact public QSOLQEC source commit used to produce the executable.
